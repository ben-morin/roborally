...
// `addUIData` takes the size of the deck the game deals from, not a player count.
const DECK_8 = 84;
const DECK_12 = 126;
...
describe('sentinel cards', () => {
  it.each([
    ...
    const [ui] = addUIData([cardId], true, 0, false, DECK_8);

    expect(ui).toEqual({ cardId, type });
  });

  it('gives a sentinel card a slot index when the row is selectable', () => {
    const ui = addUIData([CardLogic.EMPTY, CardLogic.EMPTY], false, 0, true, DECK_8);
...
describe('real cards', () => {
  it('maps each card id to its type and priority', () => {
    const ui = addUIData([U_TURN, TURN_RIGHT, STEP_FORWARD, STEP_3], false, 0, false, DECK_8);
  ...
  it('reads the twelve-player deck when the game deals from the bigger one', () => {
    // Card 7 sits in turn-right for an 8-player deck but still inside u-turn for the
    // larger one, so the deck size genuinely changes what gets rendered.
    expect(addUIData([7], false, 0, false, DECK_8)[0].type).toBe('turn-right');
    expect(addUIData([7], false, 0, false, DECK_12)[0].type).toBe('u-turn');
  });

  it("marks hand cards 'available' and played cards 'played'", () => {
    expect(addUIData([U_TURN], true, 0, false, DECK_8)[0].class).toBe('available');
    expect(addUIData([U_TURN], false, 0, false, DECK_8)[0].class).toBe('played');
  });
  ...
  it('falls back to an empty card and warns for an id outside the deck', () => {
    ...
    const [ui] = addUIData([84], false, 0, false, DECK_8);
  ...
  it('skips undefined and null entries entirely, leaving a bag with only the id', () => {
    const ui = addUIData([undefined, null], false, 0, false, DECK_8);
...
describe('locked slots', () => {
  ...
  it('locks the trailing slots, counting back from the last register', () => {
    const ui = addUIData(register, false, 2, false, DECK_8);
  ...
  it('locks nothing when the damage count is zero or false', () => {
    for (const locked of [0, false]) {
      const ui = addUIData(register, false, locked, false, DECK_8);
      expect(ui.some((c) => c.locked)).toBe(false);
    }
  });

  it('locks every slot once damage reaches the register size', () => {
    const ui = addUIData(register, false, GameLogic.CARD_SLOTS, false, DECK_8);
...
describe('cards already placed in a register', () => {
  it('greys out a hand card whose id is already chosen', () => {
    ...
    const ui = addUIData([U_TURN, TURN_RIGHT], true, 0, false, DECK_8, chosen);
  ...
  it('never marks a played row as chosen, even when the id is in the set', () => {
    ...
    const ui = addUIData([TURN_RIGHT], false, 0, false, DECK_8, new Set([TURN_RIGHT]));
  ...
  it('combines locked and chosen on the same card', () => {
    const ui = addUIData([U_TURN, TURN_RIGHT], true, 1, false, DECK_8, new Set([TURN_RIGHT]));

    // CARD_SLOTS(5) - locked(1) = 4, so only index >= 4 locks — index 1 does not.
    expect(ui[1].class).toBe('available chosen');

    const full = addUIData(
      Array(5).fill(TURN_RIGHT),
      true,
      1,
      false,
      DECK_8,
...
describe('shape', () => {
  it('returns one bag per input card, in order, and never mutates the input', () => {
    ...
    const ui = addUIData(cards, true, 0, true, DECK_8);
  ...
  it('returns an empty list for an empty register', () => {
    expect(addUIData([], true, 0, true, DECK_8)).toEqual([]);
  });
});
