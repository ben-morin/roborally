...
const login = () => {
  ...
  Meteor.loginWithPassword(loginSelector, password, (error, result) => {
    if (error) {
      loginButtonsSession.set('needsEmailVerification', error.error === 'email-not-verified');
      loginButtonsSession.errorMessage(error.reason || "Unknown error");
    } else {
    ...
    if (error) {
      if (error.error === 'email-not-verified') {
        ...
        const emailInput = document.getElementById('login-email')
          || document.getElementById('login-username-or-email');
        if (emailInput && email) emailInput.value = email;
        loginButtonsSession.infoMessage('Account created! Please check your inbox and verify your email before logging in.');
      } else {
        loginButtonsSession.errorMessage(error.reason || "Unknown error");
      }
    } else {
...
const forgotPassword = () => {
  ...
  Meteor.call('isEmailAvailable', (error, available) => {
    if (!available) {
      loginButtonsSession.errorMessage("Password reset is unavailable.");
      return;
    }
...
Template.loginButtons.events({
  'click #login-name-link, click #login-sign-in-link': () => {
    // if dropDown is already visible, close it otherwise open it
  ...
  'keypress #login-name-link, keypress #login-sign-in-link, ': event => {
    if (event.keyCode === 13) {
      if (loginButtonsSession.get('dropdownVisible')) {
        loginButtonsSession.closeDropdown();
      } else {
...
Template._loginButtonsLoggedInDropdown.events({
  ...
  'keypress #login-buttons-open-change-password': event => {
    if (event.keyCode === 13) {
      loginButtonsSession.resetMessages();
      loginButtonsSession.set('inChangePasswordFlow', true);
    }
...
Template._loginButtonsLoggedOutDropdown.events({
  ...
  'click #login-buttons-resend-verification': event => {
    event.preventDefault();
    loginButtonsSession.resetMessages();
    ...
    if (!email) {
      loginButtonsSession.set('needsEmailVerification', true);
      loginButtonsSession.errorMessage('Please enter your email address.');
      return;
    }
    ...
    Meteor.call('resendVerificationEmail', email, (error) => {
      if (error) {
        ...
        loginButtonsSession.set('needsEmailVerification', false);
        loginButtonsSession.infoMessage('Verification email sent! Please check your inbox.');
      }
    });
  },
  ...
  'keypress #forgot-password-email, keypress #login-buttons-forgot-password': event => {
    if (event.keyCode === 13) {
      event.preventDefault();
      forgotPassword();
    }
  ...
  'keypress #back-to-login-link, keypress #back-to-login-button': event => {
    if (event.keyCode === 13) {
      event.preventDefault();
      backToLogin();
    }
  ...
  'click #back-to-login-link, click #back-to-login-button': backToLogin,
...
Template._loginButtonsLoggedOutPasswordService.helpers({
  ...
  needsEmailVerification: () => loginButtonsSession.get('needsEmailVerification'),
});
...
Template._loginButtonsChangePassword.events({
  'keypress #login-old-password, keypress #login-password, keypress #login-password-again, keypress #login-buttons-do-change-password': event => {
    if (event.keyCode === 13)
      changePassword();
  },
  'click #login-buttons-do-change-password': changePassword,
  'click #login-buttons-cancel-change-password': () => {
    loginButtonsSession.resetMessages();
    loginButtonsSession.set('inChangePasswordFlow', false);
    loginButtonsSession.set('dropdownVisible', true);
  },
  'keypress #login-buttons-cancel-change-password': event => {
    if (event.keyCode === 13) {
      loginButtonsSession.resetMessages();
      loginButtonsSession.set('inChangePasswordFlow', false);
      loginButtonsSession.set('dropdownVisible', true);
