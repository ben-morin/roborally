...
export class Area {
  // The recipes run with `this` bound to the board being built (see Board.addArea). The
  // import above is type-only, so this module still has no load-time edge to board.ts.
  static start: Record<string, () => void> & ThisType<Board> = {
    simple() {
      this.addWall(2, 0, 'up');
      this.addWall(4, 0, 'up');
      this.addWall(7, 0, 'up');
    ...
    dev_test() {
      ...
      this.addStart(4, 3, 'up');
    },
  };

  static course: Record<string, () => void> & ThisType<Board> = {
    test_pit() {
      this.setVoid(1, 3);
    },
    ...
    canner_row() {
      ...
      this.addDoubleLaser(9, 9, 'u', 1);
    },
  };

  static boundaryWalls(this: Board) {
    this.addWall(2, 0, 'u');
    this.addWall(4, 0, 'u');
    this.addWall(7, 0, 'u');
    this.addWall(9, 0, 'u');
