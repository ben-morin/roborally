...
//
// The typed predicate is what makes `Infer` read `AnyOf(Date, Null)` as `Date | null`, and
// the name has to go on through `Object.assign`: `Matcher<T>` declares no `name`, and
// casting to a type that has one would make `Null` an intersection, which `Infer` reads as
// `unknown`.
export const Null = Match.Where((x): x is null => x === null);
Object.assign(Null, { name: 'Null' });
