...
async function executeRollers(players) {
  const roller_moves = [];
...
async function executeExpressRollers(players) {
  const roller_moves = [];
  for (const player of players) {
    //check if is on roller
    const tile = await player.tileAsync();
...
async function executeGears(players) {
  for (const player of players) {
    const tile = await player.tileAsync();
...
async function executePushers(players) {
...
async function executeLasers(players) {
  let victims = [];
...
async function shootRobotLaserAsync(players, player, victims) {
  const step = { x: 0, y: 0 };
